...
class Manage_anomali extends CI_Controller {
	...
	{	
		...
		$data['main_view'] = 'user/input_anomali_user';
	...
	public function simpan()
	{
		...
		{ 		
			if ($this->session->userdata('login') == "sudah login") {
				...
				$data['main_view'] = 'user/anomali';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
				redirect(base_url('index.php/user/auth'));
		...
		{ 	
			if ($this->session->userdata('login') == "sudah login") {
				...
				$data['main_view'] = 'user/anomali_SUTT';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		{ 		
			if ($this->session->userdata('login') == "sudah login") {
				...
				$data['karyawan'] = $this->model_data->select_join_where('anomali', 'basecamp', $join, 'gardu_induk', $join2, 'bay', $join3, 'peralatan_ano_bay', $join4, 'ano_gi', $join5, $where);
				$data['main_view'] = 'user/anomali';
				$data['main_view'] = 'user/rapat_anomali_gi';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		{ 
			if ($this->session->userdata('login') == "sudah login") {
				...
				$where = array(
						...
						'rencana_penanganan' => "",);
				$data['karyawan'] = $this->model_data->select_join_where_SUTT('anomali', 'basecamp', $join, 'gardu_induk', $join2, 'sutet', $join3, 'ano_sutet', $join4, $where);
				$data['main_view'] = 'user/rapat_anomali_SUTT';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		public function ubah_anomali() {
			if ($this->session->userdata('login') == "sudah login") {
				if($this->session->userdata('status') == "pegawai"){	$a = $this->uri->segment(4);
		$data['key'] = $this->model_data->getanomali($a);
		$data['main_view']= 'user/input_hasil_rapat.php';
		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
		...
		public function updates_anomali()
		{
			...
			$data = array(
				...
				if ($this->db->affected_rows() > 0){
						...
						redirect('user/manage_anomali/rapat_gi');
				}else{
		...
		$data['main_view']= 'user/input_hasil_rapat_SUTT.php';
		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		public function updates_anomali_SUTT()
		{
			...
			$data = array(
				...
				if ($this->db->affected_rows() > 0){
						$this->session->set_flashdata('notif', 'berhasil');
						redirect('user/manage_anomali/rapat_SUTT');
				}else{
						$this->session->set_flashdata('notif', 'gagal');
						redirect('user/manage_anomali/rapat_SUTT');
				}}else{
		...
		public function hapus_anomali_gi() {
			...
			if ($go) {
				...
				redirect('user/manage_anomali/anomali_gi');
			} else {
		...
		public function hapus_anomali_SUTT() {
			...
			if ($go) {
				$this->session->set_flashdata('hapus', 'berhasil');
				redirect('user/manage_anomali/anomali_SUTT');
			} else {
				$this->session->set_flashdata('hapus', 'gagal');
				redirect('user/manage_anomali/anomali_SUTT');
			}}else{
		...
		public function handled_anomali_gi() {
			if ($this->session->userdata('login') == "sudah login") {
				if($this->session->userdata('status') == "pegawai"){
						$a = $this->uri->segment(4);
		$data['key'] = $this->model_data->getanomali($a);
		$data['main_view']= 'user/input_handled_gi.php';
		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		public function updates_handled_anomali()
		{
			...
			$data = array(
				...
				if ($this->db->affected_rows() > 0){
						$this->session->set_flashdata('notif', 'berhasil');
						redirect('user/manage_anomali/handled_gi');
				}else{
						$this->session->set_flashdata('notif', 'gagal');
						redirect('user/manage_anomali/handled_gi');
				}}else{
		...
		{ 
			...
			$where = array(
				...
				$data['karyawan'] = $this->model_data->select_join_where('anomali', 'basecamp', $join, 'gardu_induk', $join2, 'bay', $join3, 'peralatan_ano_bay', $join4, 'ano_gi', $join5, $where);
				$data['main_view'] = 'user/handled_gi';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		{ 	
			...
			$where = array(
				...
				$data['karyawan'] = $this->model_data->select_join_where_SUTT('anomali', 'basecamp', $join, 'gardu_induk', $join2, 'sutet', $join3, 'ano_sutet', $join4, $where);
				$data['main_view'] = 'user/handled_SUTT';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		public function handled_anomali_SUTT() {
			if ($this->session->userdata('login') == "sudah login") {
				...
				$a = $this->uri->segment(4);
		$data['key'] = $this->model_data->getanomali_sutt($a);
		$data['main_view']= 'user/input_handled_SUTT.php';
		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
			}else{
		...
		public function updates_handled_anomali_SUTT()
		{
			...
			$data = array(
				...
				if ($this->db->affected_rows() > 0){
						$this->session->set_flashdata('notif', 'berhasil');
						redirect('user/manage_anomali/handled_SUTT');
				}else{
						$this->session->set_flashdata('notif', 'gagal');
						redirect('user/manage_anomali/handled_SUTT');
				}}else{
		...
		{ 	
			if ($this->session->userdata('login') == "sudah login") {
				...
				$data['karyawan'] = $this->model_data->select_join_where_SUTT('anomali', 'basecamp', $join, 'gardu_induk', $join2, 'sutet', $join3, 'ano_sutet', $join4, $where);
				$data['main_view'] = 'user/report_SUTT';
        		$this->load->view('user/template_user', $data);}else{
					redirect(base_url('index.php/user/auth'));
				}
			}else{
		...
		{ 
			...
			$where = array(
				...
				$data['main_view'] = 'user/data_pegawai';
        		$this->load->view('user/template_user', $data);}else{
				redirect(base_url('index.php/user/auth'));
			}
		} else {
...
/* Location: ./application/controllers/user/manage_anomali.php */